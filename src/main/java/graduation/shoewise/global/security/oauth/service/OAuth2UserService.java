...
public class OAuth2UserService extends DefaultOAuth2UserService {
    ...
    private OAuth2User process(OAuth2UserRequest userRequest, OAuth2User oAuth2User) {
        ...
        log.info("OAuth2 Login - process 카카오냐 네이버냐: " + providerType.toString()); // NAVER or KAKAO 출력

        OAuth2UserInfo userInfo = OAuth2UserInfoFactory.getOAuth2UserInfo(providerType, oAuth2User.getAttributes());
        log.info("userInfo 유저정보좀 : " + userInfo.getProviderId() + " " + userInfo.getEmail() + " " + userInfo.getName());
        ...
        if (savedUser != null) { // 이미 가입한 경우
            log.info("OAuth2 - process 이미 가입함 : " + providerType.toString());
